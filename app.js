...
async function saveState() { await dbSet(APP_STATE_KEY, state); }
...
function renderDashboard() {
  ...
  const batchMap = {};
  due.forEach(w => batchMap[w.createdAt] = (batchMap[w.createdAt] || 0) + 1);
  document.getElementById('batchSummary').innerHTML = Object.keys(batchMap).sort().map(d => `<p>${d} 批次：<strong>${batchMap[d]}</strong> 个</p>`).join('') || '<p class="muted">今天没有到期批次。</p>';
}
...
function updateReviewCard() {
  ...
  if (currentReviewIndex >= currentReviewQueue.length) {
    ...
    renderDashboard(); renderLog(); renderLibrary(); renderCalendar();
    return;
  }
...
async function applyRating(wordId, rating, attempt) {
  ...
  if (rating === 'Easy') {
    ...
    newStage = oldStage; nextDate = addDays(todayStr(), 1); item.wrongCount = (item.wrongCount || 0) + 1;
  } else if (rating === 'Again') {
    newStage = 0; nextDate = todayStr(); item.wrongCount = (item.wrongCount || 0) + 1;
  }
  item.stageIndex = newStage;
  item.nextReviewDate = nextDate;
  item.lastReviewDate = todayStr();
  state.logs.unshift({ id: uuid(), ts: new Date().toLocaleString('zh-CN'), word: item.word, rating, oldStage, newStage, nextDate, attempt: attempt || '' });
  await saveState();
  currentReviewIndex += 1;
  renderDashboard(); updateReviewCard();
}
async function addWord(entry) {
  state.words.push({
    id: uuid(), word: entry.word, meaning: entry.meaning, example: entry.example || '', tags: entry.tags || [],
    createdAt: todayStr(), nextReviewDate: todayStr(), stageIndex: 0, wrongCount: 0, lastReviewDate: null
  });
  await saveState();
}
...
function renderLibrary() {
  ...
  if (state.words.length === 0) { container.innerHTML = '<div class="muted">词库为空</div>'; return; }
  container.innerHTML = state.words.slice().sort((a, b) => b.createdAt.localeCompare(a.createdAt)).map(w => `
    <div class="list-item">
      ...
      <div class="word-actions">
        <button class="btn small-btn" data-action="edit-word" data-id="${w.id}">编辑</button>
        <button class="btn small-btn danger-outline" data-action="delete-word" data-id="${w.id}">删除</button>
      </div>
    </div>`).join('');
}
function renderLog() {
  const container = document.getElementById('logList');
  if (state.logs.length === 0) { container.innerHTML = '<div class="muted">暂无记录</div>'; return; }
  container.innerHTML = state.logs.slice(0, 200).map(log => `
    <div class="list-item">
      <div class="word-head"><strong>${escapeHtml(log.word)}</strong><div>${log.rating}</div></div>
...
function renderCalendar() {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    html += `<div class="day"><div class="date">${day}</div><div class="small ${dueCount > 0 ? '' : 'muted'}">复习: ${dueCount}</div><div class="small ${newCount > 0 ? '' : 'muted'}">新词: ${newCount}</div></div>`;
  }
  grid.innerHTML = html;
}
...
async function saveEditWord() {
  ...
  const item = state.words.find(w => w.id === editingWordId);
  if (!item) return;
  ...
  const tags = document.getElementById('editTagInput').value.split(',').map(s => s.trim()).filter(Boolean);
  if (!word || !meaning) { showToast('请填写单词和释义'); return; }
  item.word = word; item.meaning = meaning; item.example = example; item.tags = tags;
  await saveState();
  closeEditModal(); renderLibrary(); renderReview(); renderDashboard();
  showToast('已保存修改');
}
async function deleteWord(wordId) {
  const item = state.words.find(w => w.id === wordId);
  if (!item) return;
  if (!confirm(`确定删除 ${item.word} 吗？`)) return;
  state.words = state.words.filter(w => w.id !== wordId);
  await saveState();
  renderLibrary(); renderReview(); renderDashboard(); renderCalendar();
  showToast('已删除');
}
function bindEvents() {
  ...
  document.getElementById('addWordBtn').addEventListener('click', async () => {
    ...
    renderDashboard(); renderLibrary(); renderCalendar();
    showToast('已添加');
  });
  document.getElementById('addBatchDemoBtn').addEventListener('click', async () => {
    ...
    for (const [word, meaning, example, tags] of demo) await addWord({ word, meaning, example, tags });
    renderDashboard(); renderLibrary(); renderCalendar();
    showToast('已导入 5 个示例');
  });
  ...
  document.querySelectorAll('#rateButtons button').forEach(btn => btn.addEventListener('click', async () => {
    const item = currentReviewQueue[currentReviewIndex];
    const attempt = document.getElementById('spellingAttempt').value.trim();
  ...
  document.getElementById('saveSettingsBtn').addEventListener('click', async () => {
    ...
    if (intervals.length === 0) { showToast('复习间隔不能为空'); return; }
    state.settings.dailyQuota = dailyQuota;
    state.settings.intervals = intervals;
    await saveState(); renderDashboard(); showToast('设置已保存');
  });
  ...
  document.getElementById('importInput').addEventListener('change', async (event) => {
    ...
    try {
      ...
      state = { settings: { ...defaultState.settings, ...parsed.settings }, words: Array.isArray(parsed.words) ? parsed.words : [], logs: Array.isArray(parsed.logs) ? parsed.logs : [] };
      await saveState(); initViews(); showToast('导入成功');
  ...
  document.getElementById('clearDemoBtn').addEventListener('click', async () => {
    ...
    await saveState(); initViews(); showToast('已清空');
  });
  document.getElementById('libraryList').addEventListener('click', async (event) => {
    ...
    if (action === 'edit-word') openEditModal(wordId);
    if (action === 'delete-word') await deleteWord(wordId);
  });
...
function initViews() {
  renderDashboard(); renderReview(); renderLog(); renderLibrary(); renderCalendar(); renderSettings(); switchTab('dashboard');
}
